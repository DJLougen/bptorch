...
## Context
Neural Blueprint Studio requires a single source of truth for neural-network architectures that serves editing, validation, shape inference, PyTorch runtime execution, and project serialization. UI libraries like React Flow maintain internal node/edge structures with visual and presentation state.
